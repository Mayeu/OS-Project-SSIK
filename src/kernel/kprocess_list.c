 ...
 */
  pls_item       *pls_search_pcb(pls * ls, pcb * p)
  {
    pls_item       *last;
 ...
 * @return NULL if not found, the pls_item otherwise
 */
  pls_item       *pls_search_pid(pls * ls, uint32_t pid)
  {
    pls_item       *last;
 ...
 */
  void            pls_item_reset(pls_item * it, uint32_t id)
  {
    it->item_id = id;
    it->next = NULL;
    pcb_reset(&(it->p));
 ...
 */
  pls_item       *pls_item_alloc(pls * ls)
  {
    uint32_t        i;
 ...
 */
  void            pls_item_cpy_pcb(pcb * p, pls_item * it)
  {
    pcb_cpy(p, &(it->p));
  }
