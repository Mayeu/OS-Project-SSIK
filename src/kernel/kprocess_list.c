...
{
  pls_item      *it, *last_it, *next_it;

  it = pls_item_alloc(ls);

  if (!it)
    return OUTOMEM;

  pls_item_cpy_pcb(p, it);
...
pls_delete_pcb(pls * ls, pcb * p)
{
  ...
  while (pcb_get_pid(&(next->p)) != pcb_get_pid(p) && !next->next)
  {
    last = next;
    next = next->next;
  }
  ...
  if (pcb_get_pid(&(next->p)) == pcb_get_pid(p))
  {
    if (last == NULL)
      ...
      last->next = next->next;
    pls_item_reset(next, next->item_id);
  }
  else
    return NOTFOUND;
 ...
 */
int32_t
pls_move_pcb(pls * src, pls * dest, pcb * p)
{
  int32_t         e;
  pls_item      *it_src;
...
empty_space(pls * ls)
{
  int             i = 0;
  kprintln
    ("DEPRECATED: empty_space is deprecated, pleaze refers to the kprocess_list.h for new functions");
  if (ls == NULL)
    return NULL;
...
search_pcb(uint32_t pid, pls * ls)
{
  int             i = 0;
  kprintln
    ("DEPRECATED: search_pcb is deprecated, pleaze refers to the kprocess_list.h for new functions");
  while (i < MAXPCB)
  {
    if (ls->ls[i].pid == pid)
    {
...
searchall(uint32_t pid)
{
  pcb            *t;
  kprintln
    ("DEPRECATED: searchall is deprecated, pleaze refers to the kprocess_list.h for new functions");
...
move(uint32_t pid, pls * src, pls * dest)
{
  ...
  kprintln
    ("DEPRECATED: move is deprecated, pleaze refers to the kprocess_list.h for new functions");
...
sort(pls * ls)
{
  ...
  pcb             tmp;
  kprintln
    ("DEPRECATED: sort is deprecated, pleaze refers to the kprocess_list.h for new functions");
  if (ls == NULL)
    return NULLPTR;
  while (!ordered)
  {
