...
dining_philosopher(int argc, char *argv[])
{
	...
	if(argc != 3)
	{
		print("Invalid arguments number: excepted number of philosophers and the number of cycles\n");
		exit(FAILNOOB);
	}
	...
	for(i=0 ; i<nb_philo ; i++)
	{
		...
		philos[i] = fourchette("philosopher", BAS_PRI, 4, (char **) philos_args[i]); 
		if(philos[i] < 1)
		{
...
int waiter(int argc, char *argv[])		//int nb_philo
{
	if(argc < 2)
		exit(FAILNOOB);
	...
	while(!end)
	{
		...
		{
			if(can_take_fork(philo_id, philo_pid, NULL, &forks_taken, nb_philo, code) == FALSE)
			{
		...
		else if(code == RELEASE)
		{
			...
			for(i=0;i<in;i++)
			{
				...
				if(code == FORK_L || code == FORK_R)
				{
					if(can_take_fork(philo_id, philo_pid, NULL, &forks_taken,nb_philo, code))
...
int philosopher(int argc, char *argv[])
{
	...
	if(argc < 4)
	{
		print("Invalid arguments number: expected waiter pid, philosopher id and number of cycles\n");
		exit(INVARG);
	}
	...
	if(recv_from_pid(&mess, INT_T, waiter_pid, 1000) != waiter_pid && mess != START)
	{
...
bool can_take_fork(int index,int philo_pid, int* forks, int* forks_taken, int nb_philo, int rlflag)
{
