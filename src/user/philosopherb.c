...
void dinner(int num_philo, int num_cycles){
  ...
  for(i=0;i<num_philo;i++){
    ...
    if(philos[i] < 1){
      ecrire("An error occured. Impossible to create the philosophers. The process will exit now.\n");
      //clean up
      for(j=0;j<i;j++)
...
void waiter(int num_philo){
  ...
  while(!end){
    ...
    case WANT_RIGHT_FORK:
      if(!right_fork((philo_id+1)%num_philo, philo_pid,forks, &forks_taken)){
	buffer[in] = recv;
	in++;
      }else
	liberer_message(recv);
...
int left_fork(int index,int philo_pid, int* forks, int* forks_taken, int num_philo){
...
int right_fork(int index, int philo_pid, int* forks, int* forks_taken){
...
unsigned int get_random(int* m_w, int* m_z) {
