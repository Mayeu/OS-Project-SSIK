...
//  msg_arg         msgarg7 = { "test wait", CHAR_PTR, 0, 15, -1, 0 };
  ...
  create_proc("init", 10, 5, (char **) params);
  create_proc("init", 11, 5, (char **) params);
  create_proc("init", 12, 5, (char **) params);
  ...
  err = (res0 == NULLPTR) &&
    ...
    (m1.sdr_pid == 0) &&
    (m1.recv_pid == 1) &&
    (m1.pri == 10) && (strcmp(m1.data, mess1) == 0)
  ...
  err = (res == OMGROXX) &&
    (m2.sdr_pid == 0) &&
    (m2.recv_pid == 1) &&
    (m2.pri == 10) && (strcmp(m2.data, mess1) == 0)
  ...
  err = (res == OMGROXX) &&
    ...
    (m->datatype == CHAR_PTR) && (pcb0->messages.length == 1);
  test_unit(err, res);
  ...
  err = (res == OMGROXX) &&
    ...
    (m3.datatype == CHAR_PTR) && (pcb0->messages.length == 0);
  test_unit(err, res);
  ...
  err = (res == OMGROXX) &&
    ...
    (mb->datatype == CHAR_PTR) && (pcb1->messages.length == 1);
  test_unit(err, res);
  ...
  err = (res == OMGROXX) &&
    ...
    (mb->datatype == CHAR_PTR) && (pcb0->messages.length == 1);
  test_unit(err, res);
  ...
  err = (res == 0) &&
    (strcmp(messres3, "other") == 0) && (pcb0->messages.length == 0);
  test_unit(err, res);
  ...
  err = (res == OMGROXX) &&
    ...
    (mb->pri == 15) &&
    ((int) mb->data == 42) &&
    (mb->datatype == INT_T) && (pcb0->messages.length == 1);
  test_unit(err, res);
  ...
  err = (res == 1) && ((int) messres4 == 42) && (pcb0->messages.length == 1);
  test_unit(err, res);
  ...
  err = (res == 0) && ((int) messres5 == 43) && (pcb0->messages.length == 0);
  test_unit(err, res);

  kprint("recv_msg by PID\t\t\t\t\t");  //RECV_MSG
  res = recv_msg(1, &msgargres6);       // process 0 wants to receive a INT_T wit prio = 15 and put it in messres5
  err = (res == 0) &&
    (strcmp(messres6, "Hello1") == 0) && (pcb1->messages.length == 0);
  test_unit(err, res);
  ...
  err = (res == FAILNOOB) && (pcb0->messages.length == 0);
  test_unit(err, res);
  c[0] = '\0';
